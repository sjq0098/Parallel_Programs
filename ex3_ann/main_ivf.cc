...
enum class MethodType {
    ...
    IVFPQ_OMP_RERANK, // OpenMP并行IVF+PQ+Rerank
    IVFPQ_PTD_RERANK, // pthread并行IVF+PQ+Rerank
    IVFPQ_RERANK      // 串行IVF+PQ+Rerank
};
...
const std::unordered_map<MethodType, std::string> method_names = {
    ...
    {MethodType::IVFPQ_OMP_RERANK, "IVF-PQ(OpenMP)+Rerank"},
    {MethodType::IVFPQ_PTD_RERANK, "IVF-PQ(pthread)+Rerank"},
    {MethodType::IVFPQ_RERANK, "IVF-PQ(Serial)+Rerank"}
};
...
std::vector<SearchResult> test_ivfpq_method(
    ...
    size_t gt_k,
    size_t k,
    size_t L = 100) // 添加L参数，默认为100，表示rerank的候选数量
{
    std::vector<SearchResult> results(query_number);
    ...
    for (size_t q = 0; q < query_number; ++q) {
        ...
        switch (method) {
            case MethodType::IVFPQ:
                res = ivfpq_search(index.get(), query, k, nprobe);
                break;
            case MethodType::IVFPQ_OMP: {
                int num_threads = omp_get_max_threads();
                res = ivfpq_search_omp(index.get(), query, k, nprobe, num_threads);
                break;
            }
            case MethodType::IVFPQ_PTD: {
                int num_threads = std::thread::hardware_concurrency();
                res = ivfpq_search_ptd(index.get(), query,k, nprobe,  num_threads);
                break;
            }
            case MethodType::IVFPQ_OMP_RERANK: {
                int num_threads = omp_get_max_threads();
                res = ivfpq_search_omp_rerank(index.get(), base, query, k, nprobe, L, num_threads);
                break;
            }
            case MethodType::IVFPQ_PTD_RERANK: {
                int num_threads = std::thread::hardware_concurrency();
                res = ivfpq_search_ptd_rerank(index.get(), base, query, k, nprobe, L, num_threads);
                break;
            }
            case MethodType::IVFPQ_RERANK: {
                res = ivfpq_search_rerank(index.get(), base, query, k, nprobe, L);
                break;
            }
            default:
    ...
    size_t k)
{
    ...
    // 测试参数
    const std::vector<size_t> nlist_values = {64, 128, 256, 512};
    const std::vector<size_t> m_values = { 16, 32,48};
    const std::vector<size_t> nprobe_values = {4, 8, 12, 16, 20, 24, 32,64,128};
    const std::vector<size_t> L_values = {100, 200, 500}; // rerank候选数量
        ...
        for (const auto& [nlist, m] : available_indices) {
            for (size_t nprobe : nprobe_values) {
                ...
                auto results = test_ivfpq_method(
                    method, nlist, m, nprobe, base, queries, groundtruth,
                    base_number, query_number, gt_k, k
                );
    ...
    // 测试带重排序的IVFPQ方法
    for (MethodType method : {MethodType::IVFPQ_RERANK, MethodType::IVFPQ_OMP_RERANK, MethodType::IVFPQ_PTD_RERANK}) {
        for (const auto& [nlist, m] : available_indices) {
            for (size_t nprobe : nprobe_values) {
                // 确保nprobe <= nlist
                if (nprobe > nlist) continue;
                ...
                for (size_t L : L_values) {
                    ...
                    auto results = test_ivfpq_method(
                        method, nlist, m, nprobe, base, queries, groundtruth,
                        base_number, query_number, gt_k, k, L
                    );
...
int main(int argc, char* argv[]) {
    ...
    if (test_method == "ivfpq" || test_method == "all") {
        run_all_ivfpq_tests(base, queries, groundtruth, base_number, query_number, gt_k, k);
    }
