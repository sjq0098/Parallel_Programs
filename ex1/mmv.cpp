...
int main(){
	...
	ofstream outputFile("experiment_results_mmv_multiple.txt");
	...
	for(size_t i=0;i<sizes.size();i++){
		...
		LARGE_INTEGER Start1=GetPer_formance_Counter();
		for(int j=0;j<1000;j++){
			Navie_Uroll_Alg(martix,vec,result);
		}
		LARGE_INTEGER End1=GetPer_formance_Counter();
		double navie_uroll_time=Get_Elapsed_Time(Start1,End1,freq);
		
		LARGE_INTEGER Start2=GetPer_formance_Counter();
		for(int j=0;j<1000;j++){
			Cache_Optimized_Alg(martix,vec,result);
		}
		LARGE_INTEGER End2=GetPer_formance_Counter();
		double cache_time=Get_Elapsed_Time(Start2,End2,freq);
		
		outputFile << "Size: " << siz << "x" << siz << endl;
		outputFile << "Naive Time: " << naive_time << " us" << endl;
		outputFile << "Naive Unroll Time: " << navie_uroll_time << " us" << endl;
		outputFile << "Cache Optimized Time: " << cache_time << " us" << endl;
		outputFile << "--------------------------" << endl;
	}
	// 关闭文件
	outputFile.close();	
