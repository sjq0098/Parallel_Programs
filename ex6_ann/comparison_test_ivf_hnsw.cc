...
#include <sys/time.h>
#include <numeric>
#include "mpi_ivf_hnsw_lightweight.h"
...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        std::cout << "=== IVF+HNSW 对比测试程序 ===" << std::endl;
    ...
    // 测试配置参数 - 与HNSW+IVF对应，但使用更保守的参数
    struct TestConfig {
        size_t nlist, nprobe, M, efConstruction, efSearch;
        string name;
    };
    
    std::vector<TestConfig> configs = {
        {64, 4, 8, 100, 50, "快速配置"},     // 减少参数复杂度
        {64, 8, 12, 100, 80, "平衡配置"},    // 简化配置
        {128, 16, 16, 150, 100, "高精度配置"} // 保守的高精度配置
    };
    
    std::vector<TestResult> results;
    
    if (rank == 0) {
        std::cout << "\n=== 开始 IVF+HNSW 测试 ===" << std::endl;
        std::cout << "算法,配置,nlist,nprobe,M,efC,efS,recall_mean,recall_std,latency_us_mean,latency_us_std,build_time_ms,repeat_count" << std::endl;
    }
    ...
    for (const auto& config : configs) {
        ...
        // 生成聚类中心
        std::string centroids_file = "temp_centroids_" + std::to_string(config.nlist) + ".bin";
        generate_centroids(base, base_number, vecdim, config.nlist, centroids_file, rank);
        
        // 构建索引
        auto build_start = std::chrono::high_resolution_clock::now();
        
        LightweightMPIIVFHNSWIndex index(vecdim, config.nlist, config.M, config.efConstruction);
        index.setEfSearch(config.efSearch);
        index.load_centroids(centroids_file);
        ...
        if (rank == 0) {
            ...
            for (size_t i = 0; i < warmup_queries; ++i) {
                auto result = index.mpi_search(test_query + i * vecdim, k, config.nprobe);
            }
        }
        MPI_Barrier(MPI_COMM_WORLD);
        ...
        if (rank == 0) {
            ...
            // 输出结果
            std::cout << "IVF+HNSW," << config.name << "," 
                      << config.nlist << "," << config.nprobe << "," << config.M << "," 
                      << config.efConstruction << "," << config.efSearch << ","
                      ...
                      << build_time << "," << repeat_runs << std::endl;
            
            // 保存详细结果
            TestResult result_record;
            result_record.algorithm = "IVF+HNSW";
