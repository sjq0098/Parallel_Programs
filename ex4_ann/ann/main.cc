...
int main(int argc, char *argv[])
{
    ...
    const size_t M = 8;
    const size_t efC = 100;
    const size_t efS = 50;
    const std::string index_path = "files/hnsw_M8_efC100.index";

    // 加载并初始化 HNSWIndex
    HNSWIndex hnsw(vecdim, index_path, base_number);
    hnsw.setEf(efS);  // 设置 efSearch = 50
    ...
    for(int i = 0; i < test_number; ++i) {
        ...
        auto res = hnsw.omp_hnsw_search(test_query + i*vecdim,k);
