...
int main(){
	...
	ofstream outputFile("experiment_results_nnp_rep_test.txt");
	...
	for (size_t i = 0; i < sizes.size(); i++){
		...
		for (int rep = 0; rep < repetitions; rep++){
			...
			start = GetPerformanceCounter();
			resultPair = PairSum(arr);
			end = GetPerformanceCounter();
			totalTimePair += GetElapsedTime(start, end, freq);
			
			start = GetPerformanceCounter();
			resultUroll = UrollSum(arr);
			end = GetPerformanceCounter();
			totalTimeUroll += GetElapsedTime(start, end, freq);
			
			start= GetPerformanceCounter();
			Blank(arr);
			end = GetPerformanceCounter();
			double copytime=GetElapsedTime(start, end, freq);
			totalTimeCopy += copytime;
			
			totalTimeNaive-=copytime;
			totalTimePair-=copytime;
			totalTimeUroll-=copytime;
		}
		
		outputFile << "Size: " << n << " elements " <<"Average copytime:"<<totalTimeCopy/repetitions<< endl;
		outputFile << "Navie result: " << resultNaive 
		<< ", Average Time: " << totalTimeNaive / repetitions << " us" << endl;
		outputFile << "Uroll result: " << resultUroll 
		<< ", Average Time: " << totalTimeUroll / repetitions << " us" << endl;
		outputFile << "Pair  result: " << resultPair  
		<< ", Average Time: " << totalTimePair / repetitions << " us" << endl;
		outputFile << "----------------------------------------" << endl;
