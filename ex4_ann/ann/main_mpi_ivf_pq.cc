...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        std::cout << "=== MPI IVF-PQ算法参数测试 ===" << std::endl;
    ...
    for (size_t nlist : nlist_values) {
        for (size_t m : m_values) {
            ...
            for (size_t ksub : ksub_values) {
                ...
                MPIIVFPQIndex index(vecdim, nlist, m, ksub, true);  // 使用内积距离
                
                if (!index.load_centroids("temp_centroids.bin") ||
                    !index.load_codebook("temp_codebook.bin")) {
                    if (rank == 0) {
                        std::cerr << "加载聚类中心或PQ码本失败!" << std::endl;
                    }
                    continue;
                }
    ...
    // 输出CSV结果
    if (rank == 0) {
        std::ofstream csv_file("results_mpi_ivf_pq.csv");
        ...
        csv_file.close();
        std::cout << "\n结果已保存到 results_mpi_ivf_pq.csv" << std::endl;
    }
